...
parser.add_argument('--test_dir',default='/DATA/luandong/data/ReID_data/trans_reid_data/night600/pytorch_2180/',type=str, help='./test_data')
#parser.add_argument('--test_dir',default='/data/yuanhui/data/AHU-Night600_RGB_update/pytorch_2180/',type=str, help='./test_data')
parser.add_argument('--name', default='IDF_ide_night600', type=str, help='save model path')
parser.add_argument('--batchsize', default=120, type=int, help='batchsize')
...
def extract_feature(model,dataloaders):
    ...
    for data in dataloaders:
        ...
        #print(count)
        if opt.linear_num > 0:
            ff = torch.FloatTensor(n,opt.linear_num*3).zero_().cuda()
        else:
...
def get_id(img_path):
    ...
    for path, v in img_path:
        ...
        camera = filename.split('_',-1)[1].split('c')[1]
        if label[0:2] == '-1':
            labels.append(-1)
        else:
...
if opt.PCB:
    ...
    #    model = PCB_test(model[1])
        ...
        model.classifier_v.classifier = nn.Sequential()
        model.classifier_e.classifier = nn.Sequential()
        model.classifier_f.classifier = nn.Sequential()
