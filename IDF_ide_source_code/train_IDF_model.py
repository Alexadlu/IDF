...
from Myloss import L_color, L_TV, L_exp, L_spa, SSIM
...
######################################################################
# Options
# --------/data/yuanhui/data/NightPerson/pytorch
...
parser.add_argument('--name',default='IDF_ide_night600', type=str, help='output model name')
parser.add_argument('--data_dir',default='/DATA/luandong/data/ReID_data/trans_reid_data/night600/pytorch_2180/',type=str, help='training dir path')
...
parser.add_argument('--total_epoch', default=120, type=int, help='total training epoch')
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    L_exp = Myloss.L_exp(16,0.6)#(16,0.55)
    L_TV = Myloss.L_TV()
    L_spa = Myloss.L_spa()  
    loss_SSIM = Myloss.SSIM()
            ...
            for data in dataloaders[phase]:
                ...
                if phase == 'val':
                    with torch.no_grad():
                        ...
                        output_v = output_dict['x_v']
                        output_e = output_dict['x_e']
                        output_f = output_dict['x_f']
                        loss_mse = output_dict['loss']
                        en_img = output_dict['en_img']
                        A = output_dict['A']
                else:
                    output_dict = model(inputs)
                ...
                if return_feature: 
                    logits_v, ff_v = output_v
                    logits_e, ff_e = output_e
                    logits_f, ff_f = output_f


                    Loss_TV = 200*L_TV(A)

                    loss_ssim = torch.mean(loss_SSIM(en_img,inputs))

                    #loss_spa = torch.mean(L_spa(en_img, inputs))

                    loss_col = 5*torch.mean(L_color(en_img))

                    loss_exp = 10*torch.mean(L_exp(en_img))

                    #loss_dce = Loss_TV + loss_ssim + loss_exp #+loss_spa #+loss_col  # ablation
                    
                    loss_dce = Loss_TV + loss_exp +loss_spa + loss_col # IDF

                    loss_kl = kl_loss(log_sm(logits_v),sm(Variable(logits_f))) + kl_loss(log_sm(logits_e),sm(Variable(logits_f)))


                    fnorm_v = torch.norm(ff_v, p=2, dim=1, keepdim=True)
                    ff_v = ff_v.div(fnorm_v.expand_as(ff_v))
                    loss_v = criterion(logits_v, labels) 
                    _, preds_v = torch.max(logits_v.data, 1)


                    fnorm_e = torch.norm(ff_e, p=2, dim=1, keepdim=True)
                    ff_e = ff_e.div(fnorm_v.expand_as(ff_e))
                    loss_e = criterion(logits_e, labels) 
                    _, preds_e = torch.max(logits_e.data, 1)


                    fnorm_f = torch.norm(ff_f, p=2, dim=1, keepdim=True)
                    ff_f = ff_f.div(fnorm_f.expand_as(ff_f))
                    loss_f = criterion(logits_f, labels) 
                    _, preds = torch.max(logits_f.data, 1)


                    loss = loss_v + loss_e + loss_f + loss_dce + loss_kl*0.1 + loss_mse*0.1
                    ...
                    if opt.circle:
                        ...
                        ff_all = torch.cat([ff_v,ff_e,ff_f],0)
                        labels_all = torch.cat([labels,labels,labels],0)
                    ...
                    loss_dce = Loss_TV + loss_exp + loss_spa + loss_col 



                    loss_kl = kl_loss(log_sm(output_v),sm(Variable(output_f))) + kl_loss(log_sm(output_e),sm(Variable(output_f)))


                    _, preds_v = torch.max(output_v.data, 1)
                    loss_v = criterion(output_v, labels)

                    _, preds_e = torch.max(output_e.data, 1)
                    loss_e = criterion(output_e, labels)

                    _, preds = torch.max(output_f.data, 1)
                    loss_f = criterion(output_f, labels)

                    loss = loss_v + loss_e + loss_f + loss_dce + loss_mse*0.1 +  loss_kl*0.1
...
if not opt.PCB:
    ignored_params = list(map(id, model.classifier_e.parameters() )) + list(map(id, model.classifier_v.parameters() )) + list(map(id, model.classifier_f.parameters() ))\
    + list(map(id, model.E_Net.parameters() )) + list(map(id, model.bfm.parameters() ))

    base_params = filter(lambda p: id(p) not in ignored_params, model.parameters())

    optimizer_ft = optim_name([
             {'params': base_params, 'lr': 0.1*opt.lr},
             {'params': model.classifier_v.parameters(), 'lr': opt.lr},
             {'params': model.classifier_e.parameters(), 'lr': opt.lr},
             {'params': model.classifier_f.parameters(), 'lr': opt.lr},
             {'params': model.bfm.parameters(), 'lr': opt.lr},
             {'params': model.E_Net.parameters(), 'lr': 0.0001},
