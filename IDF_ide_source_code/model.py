...
class BFM(nn.Module):
    def __init__(self, input_features):
        ...
        self.fuse_in = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(input_features//2, 2048),
            nn.ReLU()
            )
        self.fuse_out = nn.Sequential(
            nn.Linear(2048, input_features//2),
            nn.ReLU(),
    ...
    def forward(self, z):
        ...
        #loss = 0 #self.criterion(self.fuse_out(compressed_z), z)
        output = {
            'z': compressed_z,
            'loss': loss,
        }
...
class ft_net(nn.Module):
    ...
    def __init__(self, class_num=751, droprate=0.5, stride=2, circle=False, ibn=False, linear_num=512):
        super(ft_net, self).__init__()
        model_ft_v = models.resnet50(pretrained=True)
        #model_ft_e = torch.hub.load('XingangPan/IBN-Net', 'resnet50_ibn_a', pretrained=True)
        model_ft_e = models.resnet50(pretrained=True)
        if ibn==True:
            model_ft = torch.hub.load('XingangPan/IBN-Net', 'resnet50_ibn_a', pretrained=True)
        # avg pooling to global pooling
        if stride == 1:
        ...
        model_ft_v.avgpool = nn.AdaptiveAvgPool2d((1,1))
        model_ft_e.avgpool = nn.AdaptiveAvgPool2d((1,1))
        
        self.model_v = model_ft_v
        self.model_e = model_ft_e

        self.bfm = BFM(4096)
        self.E_Net  = enhance_net_nopool()

        self.classifier_v = ClassBlock(2048, class_num, droprate, linear=linear_num, return_f = circle)
        self.classifier_e = ClassBlock(2048, class_num, droprate, linear=linear_num, return_f = circle)
        self.classifier_f = ClassBlock(2048, class_num, droprate, linear=linear_num, return_f = circle)

        

        self.img_name = 'IDF_img'
    def forward(self, x):
        imshow(x,self.img_name+'_x')

        en_img, A = self.E_Net(x)
        imshow(en_img,self.img_name+'_en_img')

        x_e = en_img + x
        imshow(x_e,self.img_name+'_x_e')

        x_v = x


        x_v = self.model_v.conv1(x_v)
        x_v = self.model_v.bn1(x_v)
        x_v = self.model_v.relu(x_v)
        x_v = self.model_v.maxpool(x_v)
        x_v = self.model_v.layer1(x_v)
        x_v = self.model_v.layer2(x_v)
        x_v = self.model_v.layer3(x_v)
        x_v = self.model_v.layer4(x_v)
        x_v = self.model_v.avgpool(x_v)
        x_v = x_v.view(x_v.size(0), x_v.size(1))


        x_e = self.model_e.conv1(x_e)
        x_e = self.model_e.bn1(x_e)
        x_e = self.model_e.relu(x_e)
        x_e = self.model_e.maxpool(x_e)
        x_e = self.model_e.layer1(x_e)
        x_e = self.model_e.layer2(x_e)
        x_e = self.model_e.layer3(x_e)
        x_e = self.model_e.layer4(x_e)
        x_e = self.model_e.avgpool(x_e)
        x_e = x_e.view(x_e.size(0), x_e.size(1))


        ev_cat = torch.cat((x_e,x_v),1)

        output = self.bfm(ev_cat)

        x_f = output['z']
        mse_loss = output['loss']

        x_v = self.classifier_v(x_v)
        x_e = self.classifier_e(x_e)
        x_f = self.classifier_f(x_f)
...
def imshow(tensor,img_name):
    vutils.save_image(tensor, './E_img/'+img_name+'.jpg', normalize=True)
